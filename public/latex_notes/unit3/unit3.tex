...
The \dfn{covariance} of two variables is defined by
\begin{equation}
...
Note that another way to write this is  (check yourself!)
\begin{equation*}
 {\rm cov}(X,Y) = E[(Y-E[Y])(X-E[X)] %= E[XY] - 2E[X]E[Y]+E[X]E[Y] = {\rm cov}(X,Y)
\end{equation*}
...
Finally, we want to estimate $\sigma_{\epsilon}^2$, which is the same as the conditional variance ${\rm var}(Y|X)$ in our model. From the definition of $\epsilon$, 
\begin{equation}
...
%%
%\[
%Z_i = \frac{X_i - \mu}{\sigma}, \quad Z_{i+1} = \frac{X_{i+1} - \mu}{\sigma},
%\]
%
%so both have mean zero and variance one. Let the correlation between successive observations be
%
%\[
%\mathrm{corr}(X_i, X_{i+1}) = \rho < 1.
%\]
%
%The conditional distribution of \(Z_{i+1}\) given \(Z_i\) under a bivariate normal assumption is
%
%\[
%Z_{i+1} | Z_i = z \sim \text{Normal}(\rho z, 1 - \rho^2).
%\]
%
%This corresponds to a regression model:
%
%\[
%\mathbb{E}[Z_{i+1} | Z_i = z] = \rho z.
%\]
%
%Because \(|\rho| < 1\), when \(Z_i\) takes on an extreme value (e.g., \(z=2\)), the expected value of \(Z_{i+1}\) is closer to zero:
%
%\[
%\mathbb{E}[Z_{i+1} | Z_i = 2] = 2\rho,
%\]
%
%which is less extreme than 2 since \(\rho < 1\). Thus, the subsequent measurement \(Z_{i+1}\) "regresses" toward the mean (here zero) relative to its predecessor \(Z_i\).
%
%**Intuition:** Extreme observations at one time point tend to be followed by observations closer to the average, because the deviation of the initial observation arises partly from random variation, which is unlikely to persist perfectly into the next observation.
%
%**Important notes:**
%
%- When \(\rho = 1\), there is perfect correlation and no regression to the mean: the subsequent observation matches the previous one exactly.
%- When \(\rho = 0\), there is no correlation and the expected next value is always the mean.
%- Regression to the mean is not a causal process; it is a natural consequence of imperfect correlation and variability.
%
%
...
\subsection{Regression to the mean}
%------------------------------------------------------------------------------------------------------------------------------------------------------
\section{$p$-values Hypothesis testing}
...
\section*{Exercises}
 % ------------------------------------------------------------------------------------------------------------------------------------------
\begin{exercise}[Bias and consistency \ding{111}]\label{ex:1}
Let
\begin{equation*}
