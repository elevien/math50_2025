...
\end{equation*}
so if we replaced $X$ with $Y$, this becomes the variance. Hence the name covariance -- this is essentially a generalization of variance to two variables. 
...
The crucial observation is that the covariance allows us to relate the parameter $\beta_1$ (the slope) in the model above to averages over $X$ and $Y$. In other words, it provides us with a means to estimate the slope from samples $(x_1,y_2),\dots,(x_n,y_n)$. 
\begin{align*}
...
\end{equation}
We call the observed values of $\epsilon$ the residuals, and they are defined by 
\begin{equation}
