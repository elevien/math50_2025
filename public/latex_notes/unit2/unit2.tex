...
\begin{align}
\overline{Y|X=0} &\sim {\rm Normal}(\beta_0,2\sigma^2/N)\\
\overline{Y|X=1} &\sim {\rm Normal}(\beta_0+\beta_1,2\sigma^2/N)
\end{align}
Thus
\begin{equation}
\widehat{\beta}_1 \sim {\rm Normal}(\beta_1,4\sigma^2/N)
\end{equation}
Let's test this
...
\begin{equation}
2{\rm se}(\widehat{\beta}_1) = 16 \sigma/\sqrt{N} = 0.1
\end{equation}
This means we want 
\begin{equation}
N > 16\sigma^2/0.1^2
...
\begin{exercise}[Normal approximation to estimator \ding{111}] 
...
\begin{exercise}[Implementing a geometric distribution and testing the variance formula \ding{111}]
