...
However, to make calculations simpler and better compare the frequentist approach we will take a prior where any value of $q$ is equally likely. This seems to be more similar to what we are doing in the frequentist approach. Our Bayesian model would then be
\begin{align*}
...
q|X_1,\dots,X_N
\end{equation*}
Since the flips are independent, this distribution depends only on $Y$, so we can alternatively obtain the posterior as the distribution of $q|Y$. 
...
&\implies  \sqrt{\frac{a^2}{4a^2(2a+1)}} = \frac{1}{2}\sqrt{\frac{1}{2a+1}}= \frac{0.1}{1.96}\\
...
\subsection{Bayesian inference for Normal mean}\label{eq:bayesnormal}
...
\end{align}
where $b_1$ and $b_0$ are the regression slope and intercept when $\bar{X}$ is treated as the predictor. In particular, $b_0 = E[\mu|\bar{X}=0]$. 

The variance and expected value of $\bar{X}$ are
\begin{equation}
...
\section{Bayesian inference for nonlinear model}
  We now discuss Bayesian inference for the general linear regression model with features (Or simply many predictors, the difference for most of this section is only in notation). Let
\begin{equation*}
...
Define a $K\times K$ matrix $\Omega$ by 
\begin{equation*}
...
\begin{exercise}[Bayesian updating \ding{111}]
Consider Bayesian inference for a Normal model with known variance as described in Section \ref{eq:bayesnormal}. 
Suppose we have $N$ data points and perform Bayesian inference to obtain a posterior distribution $\mu|\bar{X}$ (we could also write this as $\mu|\hat{\mu}$ since $\hat{\mu}  = 1/N\sum_{i=1}^NX_i= \bar{X}$). Now obtain $K$ new data points $X_{N+1},\dots,X_{N+K}$ and we want update our posterior. We could either: 
\begin{itemize}
\item Compute the sample average $\bar{X}_{N+k} =1/(N+K) \sum_{i=1}^{N+K}X_i$ of all $N+K$ data points and compute the posterior distribution $\mu|\bar{X}_{N+K}$, OR
\item We could use the posterior distribution from the first $N$ samples as a new prior and then find a new posterior distribution based on the new samples. 
\end{itemize}
Determine if these are the same or different. 
