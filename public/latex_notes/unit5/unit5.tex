...
\begin{align*}
 ...
 &  =  \left(\beta_1 (x+1)+ \beta_2 X_2 + \J_{1,2} (x+1)X_2\right)- \left(\beta_1 x + \beta_2 X_2 + J_{1,2} x X_2\right)\\
 &= \beta_1 + J_{1,2}X_2
\end{align*}
...
%\end{proof}
 ...
 Often we want to summarize how different frequencies are represented in our data, but we don't particularly care about whether they come form the $\sin$ or $\cos$ terms. Notice that the sine functions shown in Theorem 1 differ from the basis function shown in example 8. This is because the interval has changed from [-L,L] to [0,L]. To achieve this, one uses the \dfn{power spectrum density}, also known as the \dfn{peridogram}, 
\begin{equation*}
